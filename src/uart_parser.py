...
class AutoRadarUARTParser:
    ...
    # Max buffer before we trim to the latest magic word.
    # Keep this small (64 KB) so a stuck parser cannot grow unbounded
    # while the OS serial buffer (4 KB) keeps dropping bytes.
    ...
    def _try_parse_one_frame(self):
        ...
        if not candidates:
            if self._looks_like_incomplete_packet():
                # Wait for more data — BUT only if there is no later magic
                # word already in the buffer.  If there is one, this packet
                # is truncated/corrupted and we should skip to the next frame
                # rather than waiting forever for bytes that will never arrive
                # (the OS serial buffer is only 4 KB and may have already
                # dropped the tail of this packet).
                next_magic = self.buffer.find(MAGIC_WORD, len(MAGIC_WORD))
                if next_magic > 0:
                    del self.buffer[:next_magic]
                    self.bad_packets += 1
                    return None  # re-enter loop; next call will parse next frame
    ...
    def _looks_like_incomplete_packet(self):
        if len(self.buffer) >= 40:
            try:
                ...
                # Allow numTLVs == 0 (empty-scene frames are valid)
    ...
    def _try_candidate_header_40(self):
        ...
        # Allow numTLVs == 0: firmware sends empty frames when the scene is
        # clear (no detected objects).  Rejecting them causes bad_packets to
        # accumulate and the parser to lose sync.
        ...
        # Empty frame — consume it cleanly and return an empty result
        if num_tlvs == 0:
            frame = self._parse_people_tracking_tlvs(header, [])
            frame["mode"] = "PEOPLE_TRACKING"
            return {
                "frame": frame,
                "packet_length": packet_length,
                "tlv_types": [],
                "tlv_length_mode": "payload_length",
                "header_mode": "SDK_40_WITH_PEOPLE_TLV",
                "mode": "PEOPLE_TRACKING",
                "score": 50,  # low score; prefer non-empty candidate if both match
            }
